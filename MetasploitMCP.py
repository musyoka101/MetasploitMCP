...
SHELL_PROMPT_RE = re.compile(
    r"([#$>]|%)\s*$"
)  # Matches common shell prompts (#, $, >, %) at end of line
...
def initialize_msf_client() -> MsfRpcClient:
    ...
    try:
        logger.debug(
            f"Attempting to create MsfRpcClient connection to {MSF_SERVER}:{msf_port} (SSL: {msf_ssl})..."
        )
        client = MsfRpcClient(
            password=MSF_PASSWORD, server=MSF_SERVER, port=msf_port, ssl=msf_ssl
        )
        ...
        raise ConnectionError(
            f"Failed to connect/authenticate to Metasploit RPC: {e}"
        ) from e
    except Exception as e:
        logger.error(
            f"An unexpected error occurred during MSF client initialization: {e}",
            exc_info=True,
        )
...
def get_msf_client() -> MsfRpcClient:
    ...
    if _msf_client_instance is None:
        logger.error(
            "Metasploit client has not been initialized. Check MSF server connection."
        )
        raise ConnectionError(
    ...
    try:
        ...
        if isinstance(console_object, MsfConsole) and hasattr(console_object, "cid"):
            ...
            if not console_id_str:
                raise ValueError(
                    "Console object created, but .cid attribute is empty or None."
                )
            ...
            raise MsfRpcError(
                f"Unexpected result from console creation: {console_object}"
            )
        ...
        if console_id_str and _msf_client_instance:  # Check client still exists
            try:
                logger.info(
                    f"Attempting to destroy Metasploit console (ID: {console_id_str})..."
                )
                # Use lambda to avoid potential issues with capture
                destroy_result = await asyncio.to_thread(
                    lambda cid=console_id_str: _msf_client_instance.consoles.destroy(
                        cid
                    )
                )
...
async def run_command_safely(
    ...
    if not (hasattr(console, "write") and hasattr(console, "read")):
        logger.error(
            f"Console object {type(console)} lacks required methods (write, read)."
        )
    ...
    try:
        ...
        output_buffer = (
            b""  # Read as bytes to handle potential encoding issues and prompt matching
        )
        ...
        while True:
            ...
            try:
                ...
                logger.warning(
                    f"Error reading from console during command '{cmd}': {read_err}"
                )
            ...
            if chunk_data:
                ...
                if prompt_bytes and MSF_PROMPT_RE.search(prompt_bytes):
                    logger.debug(
                        f"Detected MSF prompt in console.read() result for '{cmd}'. Command likely complete."
                    )
                    break
                ...
                if MSF_PROMPT_RE.search(output_buffer):
                    logger.debug(
                        f"Detected MSF prompt at end of buffer for '{cmd}'. Command likely complete."
                    )
                    break

            # Fallback Completion Check: Inactivity timeout
            elif (
                (current_time - last_data_time) > SESSION_READ_INACTIVITY_TIMEOUT
            ):  # Use a shorter inactivity timeout here
                logger.debug(
                    f"Console inactivity timeout ({SESSION_READ_INACTIVITY_TIMEOUT}s) reached for command '{cmd}'. Assuming complete."
                )
                break
...
def _parse_options_gracefully(
    ...
    if isinstance(options, str):
        ...
        try:
            ...
            logger.info(
                f"Successfully converted string options to dict: {parsed_options}"
            )
            return parsed_options

        except Exception as e:
            raise ValueError(
                f"Failed to parse options string '{options}': {e}. Expected format: 'key=value,key2=value2' or dict {{'key': 'value'}}"
            )
    ...
    try:
        ...
        raise ValueError(
            f"Options must be a dictionary or comma-separated string format 'key=value,key2=value2'. Got {type(options)}: {options}"
        )
...
async def _get_module_object(module_type: str, module_name: str) -> Any:
    ...
    try:
        ...
        logger.debug(
            f"Successfully retrieved module object for {module_type}/{base_module_name}"
        )
        return module_obj
        ...
        if (
            ...
            logger.error(
                f"Module {module_type}/{base_module_name} (from input {module_name}) not found."
            )
            raise ValueError(
                f"Module '{module_name}' of type '{module_type}' not found."
            ) from e
        else:
            logger.error(
                f"MsfRpcError getting module {module_type}/{base_module_name}: {e}"
            )
...
async def _set_module_options(module_obj: Any, options: Dict[str, Any]):
    ...
    logger.debug(
        f"Setting options for module {getattr(module_obj, 'fullname', '')}: {options}"
    )
    for k, v in options.items():
        ...
        try:
            ...
            await asyncio.to_thread(
                lambda key=k, value=v: module_obj.__setitem__(key, value)
            )
            ...
            raise ValueError(
                f"Failed to set option '{k}' to '{original_value}': {e}"
            ) from e
...
async def _execute_module_rpc(
    ...
    """
    client = get_msf_client()
    module_obj = await _get_module_object(
        module_type, module_name
    )  # Handles path variants
    ...
    if module_type == "exploit" and payload_spec:
        if isinstance(payload_spec, str):
            ...
            try:
                ...
                logger.error(
                    f"Failed to prepare payload object for '{payload_name}': {e}"
                )
                return {
                    "status": "error",
            ...
            return {
                "status": "error",
                "message": "Invalid payload specification format.",
            }
    ...
    try:
            ...
            if module_type == "exploit" and "handler" in full_module_path:
                ...
                for jid, jinfo in jobs_list.items():
                    if (
                        ...
                        and jinfo.get("datastore", {}).get("LHOST")
                        == module_options.get("LHOST")
                        and jinfo.get("datastore", {}).get("LPORT")
                        == module_options.get("LPORT")
                        and jinfo.get("datastore", {}).get("PAYLOAD")
                        == payload_name_for_log
                    ):
                        logger.info(
                            f"Found probable handler job {jid} matching parameters."
                        )
                        return {
                            "status": "success",
        ...
        # Skip polling for multi/handler — listeners don't produce sessions immediately.
        found_session_id = None
        is_handler = "multi/handler" in full_module_path.lower()
        if module_type == "exploit" and uuid and not is_handler:
            start_time = asyncio.get_event_loop().time()
            logger.info(
            ...
            while (
                ...
                try:
                    ...
                    for s_id, s_info in sessions_list.items():
                        ...
                        s_id_str = str(s_id)
                        if isinstance(s_info, dict) and str(
                            s_info.get("exploit_uuid")
                        ) == str(uuid):
                            found_session_id = s_id  # Keep original type from list keys
                            logger.info(
                                f"Found matching session {found_session_id} for job {job_id} (UUID: {uuid})"
                            )
                    ...
                    logger.error(
                        f"Unexpected error during polling: {poll_e}", exc_info=True
                    )
                    break
            ...
            if found_session_id is None:
                logger.warning(
                    f"Polling timeout ({EXPLOIT_SESSION_POLL_TIMEOUT}s) reached for job {job_id}, no matching session found."
                )
        ...
        if "missing required option" in error_str or "invalid option" in error_str:
            ...
            return {
                "status": "error",
                "message": f"Invalid payload specified: {payload_name_for_log or 'None'}. {e}",
            }
        return {"status": "error", "message": f"Error running {full_module_path}: {e}"}
    except Exception as e:
        logger.exception(
            f"Unexpected error executing module {full_module_path} via RPC"
        )
        return {
            "status": "error",
...
async def _execute_module_console(
    ...
    logger.info(
        f"Executing {full_module_path} synchronously via console (command: {command})..."
    )
        ...
        try:
            ...
            if session_match:
                try:
                    ...
                    session_opened_line = session_match.group(
                        0
                    )  # The matched line segment
                    logger.info(
                        f"Detected session {session_id} opened in console output."
                    )
                except (ValueError, IndexError):
                    logger.warning(
                        "Found session opened pattern, but failed to parse ID."
                    )
            ...
            if (
                ...
                and any(
                    term in module_output.lower()
                    for term in ["session opened", "sending stage"]
                )
            ):
            ...
            if any(
                ...
                status = (
                    "error" if status != "warning" else status
                )  # Don't override warning if session might have opened
                message = f"{module_type.capitalize()} module {full_module_path} execution via console appears to have failed. Check output."
                logger.error(
                    f"Failure detected in console output for {full_module_path}."
                )
            ...
            logger.exception(
                f"Unexpected error during console execution of {full_module_path}"
            )
            return {
                "status": "error",
...
async def list_exploits(search_term: str = "") -> List[str]:
    ...
    try:
        ...
        logger.debug(
            f"Calling client.modules.exploits with {RPC_CALL_TIMEOUT}s timeout..."
        )
        ...
        if search_term:
            ...
            limit = 200
            logger.info(
                f"Found {count} exploits matching '{search_term}'. Returning max {limit}."
            )
...
async def list_payloads(platform: str = "", arch: str = "") -> List[str]:
    ...
    client = get_msf_client()
    logger.info(
        f"Listing payloads (platform: '{platform or 'Any'}', arch: '{arch or 'Any'}')"
    )
    try:
        # Add timeout to prevent hanging on slow/unresponsive MSF server
        logger.debug(
            f"Calling client.modules.payloads with {RPC_CALL_TIMEOUT}s timeout..."
        )
        ...
        if platform:
            ...
            filtered = [
                p
                for p in filtered
                if p.lower().startswith(plat_lower + "/")
                or f"/{plat_lower}/" in p.lower()
            ]
        if arch:
...
async def generate_payload(
    ...
    client = get_msf_client()
    logger.info(
        f"Generating payload '{payload_type}' (Format: {format_type}) via RPC. Options: {options}"
    )
    ...
    try:
        ...
        async def update_runoption(key, value):
            if value is None:
                return
            await asyncio.to_thread(
                lambda k=key, v=value: payload.runoptions.__setitem__(k, v)
            )
        ...
        if not isinstance(raw_payload_bytes, bytes):
            ...
            if isinstance(raw_payload_bytes, dict) and raw_payload_bytes.get("error"):
                error_msg = raw_payload_bytes.get(
                    "error_message", str(raw_payload_bytes)
                )
            return {
                "status": "error",
                "message": f"Payload generation failed: {error_msg}",
            }
        ...
        try:
            ...
            logger.error(
                f"Failed to create payload save directory {PAYLOAD_SAVE_DIR}: {e}"
            )
            return {
                "status": "error",
        ...
        if output_filename:
            sanitized = re.sub(
                r"[^a-zA-Z0-9_.\-]", "_", os.path.basename(output_filename)
        ...
        if "invalid payload type" in error_str or "unknown module" in error_str:
            return {
                "status": "error",
                "message": f"Invalid payload type: {payload_type}",
            }
        elif "missing required option" in error_str or "invalid option" in error_str:
            missing = (
                getattr(payload, "missing_required", [])
                if "payload" in locals()
                else []
            )
            return {
                "status": "error",
                "message": f"Missing/invalid options for payload {payload_type}: {e}",
                "missing_required": missing,
            }
        ...
        logger.exception(
            f"AttributeError during payload generation for '{payload_type}': {e}"
        )
        ...
        logger.exception(
            f"Unexpected error during payload generation for '{payload_type}'."
        )
        return {
            "status": "error",
...
async def run_exploit(
    ...
    """
    logger.info(
        f"Request to run exploit '{module_name}'. Job: {run_as_job}, Check: {check_vulnerability}, Payload: {payload_name}"
    )
    ...
    if check_vulnerability:
        ...
        try:
            ...
            is_vulnerable = (
                "appears vulnerable" in output
                or "is vulnerable" in output
                or "+ vulnerable" in output
            )
            # Check for negative indicators (more reliable sometimes)
            is_not_vulnerable = (
                "does not appear vulnerable" in output
                or "is not vulnerable" in output
                or "target is not vulnerable" in output
                or "check failed" in output
            )
            ...
            if is_not_vulnerable or (
                not is_vulnerable and check_result.get("status") == "error"
            ):
                logger.warning(
                    f"Check indicates target is likely not vulnerable to {module_name}."
                )
                return {
                    "status": "aborted",
                    "message": f"Check indicates target not vulnerable. Exploit not attempted.",
                    "check_output": check_result.get("module_output"),
                }
            elif not is_vulnerable:
                logger.warning(
                    f"Check result inconclusive for {module_name}. Proceeding with exploit attempt cautiously."
                )
            else:
                logger.info(
                    f"Check indicates target appears vulnerable to {module_name}. Proceeding."
                )
            # Optionally return check output here if needed by the agent

        except Exception as chk_e:
            logger.warning(
                f"Vulnerability check failed for {module_name}: {chk_e}. Proceeding with exploit attempt."
            )
...
async def run_post_module(
    ...
    """
    logger.info(
        f"Request to run post module {module_name} on session {session_id}. Job: {run_as_job}"
    )
    ...
    try:
        ...
        if str(session_id) not in current_sessions:
            logger.error(
                f"Session {session_id} not found for post module {module_name}."
            )
            return {
                "status": "error",
        ...
        logger.error(
            f"Failed to validate session {session_id} before running post module: {e}"
        )
...
async def run_auxiliary_module(
    ...
    """
    logger.info(
        f"Request to run auxiliary module {module_name}. Job: {run_as_job}, Check: {check_target}"
    )
    module_options = options or {}
    ...
    if check_target:
        ...
        try:
            ...
            logger.info(
                f"Auxiliary check result: {check_result.get('status')} - {check_result.get('message')}"
            )
            output = check_result.get("module_output", "").lower()
            # Generic check for positive outcome (aux check output varies widely)
            is_positive = (
                "host is likely vulnerable" in output
                or "target appears reachable" in output
                or "+ check" in output
            )
            is_negative = (
                "host is not vulnerable" in output
                or "target is not reachable" in output
                or "check failed" in output
            )

            if is_negative or (
                not is_positive and check_result.get("status") == "error"
            ):
                logger.warning(
                    f"Check indicates target may not be suitable for {module_name}."
                )
                return {
                    "status": "aborted",
                    "message": f"Check indicates target unsuitable. Module not run.",
                    "check_output": check_result.get("module_output"),
                }
            elif not is_positive:
                logger.warning(
                    f"Check result inconclusive for {module_name}. Proceeding with run."
                )
            else:
            ...
            logger.warning(
                f"Check failed for auxiliary {module_name}: {chk_e}. Proceeding with run attempt."
            )
...
async def list_active_sessions() -> Dict[str, Any]:
    ...
    try:
        logger.debug(
            f"Calling client.sessions.list with {RPC_CALL_TIMEOUT}s timeout..."
        )
...
async def send_session_command(
    ...
    try:
        ...
        session = await asyncio.to_thread(
            lambda: client.sessions.session(session_id_str)
        )
        if not session:
            logger.error(
        ...
        if session_type == "meterpreter":
            ...
            logger.debug(
                f"Using session.run_with_output for Meterpreter session {session_id}"
            )
            try:
                ...
                status = "timeout"
                message = (
                    f"Meterpreter command timed out after {timeout_seconds} seconds."
                )
                logger.warning(
                    f"Command '{command}' timed out on Meterpreter session {session_id}"
                )
                ...
                logger.error(
                    f"Error during Meterpreter run_with_output for command '{command}': {run_err}"
                )
            ...
            try:
                ...
                while True:
                    ...
                    if chunk:
                        ...
                        if SHELL_PROMPT_RE.search(output_buffer):
                            logger.debug(
                                f"Detected shell prompt for command '{command}'."
                            )
                            status = "success"
                        ...
                        logger.debug(
                            f"Shell inactivity timeout ({SESSION_READ_INACTIVITY_TIMEOUT}s) reached for command '{command}'. Assuming complete."
                        )
                ...
                if command.strip().lower() == "exit":
                    logger.warning(
                        f"Error occurred after sending 'exit' to shell {session_id}: {run_err}. This might be expected as session closes."
                    )
                    ...
                    logger.error(
                        f"Error during Shell write/read loop for command '{command}': {run_err}"
                    )
...
async def list_listeners() -> Dict[str, Any]:
    ...
    try:
        ...
        for job_id, job_info in jobs.items():
            ...
            if isinstance(job_info, dict):
                ...
                # Secondary check: presence of typical handler options
                    ...
                    "lhost" in datastore and "lport" in datastore
                ):
                    is_handler = True
                    logger.debug(
        ...
        return {
            "status": "error",
            "message": f"Unexpected server error listing jobs: {e}",
        }
...
async def start_listener(
    ...
    """
    logger.info(
        f"Request to start listener for {payload_type} on {lhost}:{lport}. ExitOnSession: {exit_on_session}"
    )
    ...
    if result.get("status") == "success":
        result["message"] = (
            f"Listener for {payload_type} started as job {result.get('job_id')} on {lhost}:{lport}."
        )
    elif (
...
async def stop_job(job_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Unexpected server error stopping job {job_id}: {e}",
        }
...
async def terminate_session(session_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if session_id_str not in current_sessions_after:
            ...
            return {
                "status": "warning",
                "message": f"Session {session_id} may not have been terminated properly.",
            }
        ...
        return {
            "status": "error",
            "message": f"Error terminating session {session_id}: {e}",
        }
    except Exception as e:
...
async def health_check():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Metasploit Service Unavailable: {e}"
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run Streamlined Metasploit MCP Server"
    )
    parser.add_argument(
        "--transport",
        choices=["http", "stdio"],
    ...
    try:
        ...
        logger.critical(
            f"CRITICAL: Failed to initialize Metasploit client on startup: {e}. Server cannot function."
        )
    ...
    if args.transport == "stdio":
        ...
        logger.info(
            f"Starting Uvicorn HTTP server on http://{args.host}:{selected_port}"
        )
